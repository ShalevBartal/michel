...
function PurchaseReport() {
  ...
  const [filters, setFilters] = useState({ date: '', category: '', houseName: '' });
  ...
  useEffect(() => {
    ...
    today.setHours(23, 59, 59, 999); // Set to the very end of today

    if (filters.date) {
      const selectedDate = new Date(filters.date);
      selectedDate.setHours(0, 0, 0, 0); // Set to the start of the selected date

      result = result.filter(purchase => {
        const purchaseDate = new Date(purchase.Date_of_purchase);
        return purchaseDate >= selectedDate && purchaseDate <= today;
      });
    }
  ...
  return (
    <div className="purchase-report-container">
      ...
      <div className="filters">
        <label htmlFor="date">From date (include the date):</label>
